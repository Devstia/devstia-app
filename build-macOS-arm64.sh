...
NODE_DOWNLOAD_PATH="${BUILD_DIR}/node-download.tar.gz"
NODE_EXTRACT_PATH="${BUILD_DIR}/node-extracted"
...
# Copy the directories src, node_modules, images and the file package.json INTO app/
rsync -av --progress \
    --exclude '.git' \
    --exclude 'build' \
    --exclude 'dist' \
...
cp "${NODE_BIN_PATH}" "${APP_BUNDLE_PATH}/Contents/Resources/app/runtime/darwin_arm64/bin/node"
...
#!/bin/bash
# Get the directory containing this script (MacOS folder)
DIR=\$(cd "\$(dirname "\$0")" && pwd)
# Set the base path for the app resources
RESOURCES_DIR="\$DIR/../Resources"
# Execute the bundled Node with the bundled main.js, setting the working directory
cd "\$RESOURCES_DIR/app" || exit 1 # Change to app directory (contains src/, node_modules/ etc)
# Run main.js using path relative to current directory (app/)
"\$RESOURCES_DIR/app/runtime/darwin_arm64/bin/node" src/main.js "\$@"
EOF
